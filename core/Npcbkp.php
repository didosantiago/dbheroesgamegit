...
class Npc {
    ...
    public function saveBatalhaNPC($idPersonagem, $idOponente){
        ...
        $sql = "SELECT * FROM npc WHERE idPersonagem = $idPersonagem AND concluido = 0 ORDER BY id DESC LIMIT 1";
        $stmt = DB::prepare($sql);
        $stmt->execute();
        $dados_npc = $stmt->fetch();
    ...
    public function getLogNPC($idPersonagem, $idGuerreiro){
        // ✅ STEP 1: Get the LATEST battle ID (active OR just finished)
        $sqlLatest = "SELECT id FROM npc 
                    WHERE idPersonagem = $idPersonagem 
                        AND idDesafiado = $idGuerreiro 
                    ORDER BY id DESC 
                    LIMIT 1";
        
        $stmtLatest = DB::prepare($sqlLatest);
        $stmtLatest->execute();
        ...
        $latestBattle = $stmtLatest->fetch();
        $latestBattleId = $latestBattle->id;
        
        // ✅ STEP 2: Get log ONLY for this specific battle ID
        $sql = "SELECT ph.* 
                FROM npc_historico as ph 
                WHERE ph.idNPC = $latestBattleId 
                ORDER BY ph.id DESC";
