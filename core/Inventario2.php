...
class Inventario {
    ...
    public function getSlots($idPersonagem) {
        ...
        // 3. Get inventory data WITH QUANTIDADE
        $sql = "SELECT pi.*, pii.id as itemStorageId, pii.idItem, pii.quantidade, i.nome, i.imagem, i.tipo, i.subtipo, i.raridade, 
        ...
        foreach($slots as $slot) {
            if(!empty($slot->idItem) && $slot->idItem != 0) {
                ...
                echo 'data-quantidade="' . ($slot->quantidade ?? 1) . '" ';
                echo 'data-adesivo="' . (isset($slot->adesivo) ? $slot->adesivo : 0) . '" ';
                echo 'data-emblema="' . (isset($slot->emblema) ? $slot->emblema : 0) . '" ';
                echo 'data-isbau="' . ($isBau ? '1' : '0') . '" ';
                echo 'style="background-image: url(\'' . BASE . 'assets/' . $bgimage . '\'); background-size: cover; position: relative;">';
                ...
                // ✅ ADD QUANTITY BADGE
                if($isConsumable && isset($slot->quantidade) && $slot->quantidade > 1) {
                    echo '<span class="quantity-badge">x' . $slot->quantidade . '</span>';
                }
    ...
    public function usarConsumivel($idInventario, $idPersonagem) {
        ...
        // Get consumable item from inventory WITH QUANTIDADE
        ...
        // ✅ ============================================
        // QUANTITY MANAGEMENT - Decrease or Delete Item
        // ✅ ============================================
        $current_quantity = $item->quantidade ?? 1;
        
        if($current_quantity > 1) {
            // ✅ Decrease quantity by 1
            $sql_update_qty = "UPDATE personagens_inventario_itens 
                            SET quantidade = quantidade - 1 
                            WHERE id = ?";
            $stmt_update_qty = DB::prepare($sql_update_qty);
            $stmt_update_qty->execute([$idInventario]);
            
            $remaining_quantity = $current_quantity - 1;
            $message = 'Item usado: ' . htmlspecialchars($item->nome) . ' (Restam: ' . $remaining_quantity . ')';
        } else {
            // ✅ Remove item completely (was last one)
            $sql_delete = "DELETE FROM personagens_inventario_itens WHERE id = ?";
            $stmt_delete = DB::prepare($sql_delete);
            $stmt_delete->execute([$idInventario]);
            
            $remaining_quantity = 0;
            $message = 'Item usado: ' . htmlspecialchars($item->nome) . ' (Último item consumido)';
        }
        ...
        return [
            ...
            'remaining_quantity' => $remaining_quantity  // ✅ Send remaining quantity to frontend
        ];
    }
    ...
    /**
     * Check if item with same name exists and return slot ID with quantity info
     * FOR STACKING ONLY - Only works with consumables
     */
    public function verificaItemIgual($nomeItem, $idPersonagem) {
        // Get item data by name
        $sql = "SELECT * FROM itens WHERE nome = ? LIMIT 1";
        $stmt = DB::prepare($sql);
        $stmt->execute([$nomeItem]);
        $item = $stmt->fetch();
        ...
        // Check if item is stackable (consumables only)
        $stackable_types = ['consumivel', 'capsula', 'comida', 'restauracao', 'pocao'];
        $is_stackable = in_array(strtolower($item->tipo ?? ''), $stackable_types);
        
        if(!$is_stackable) {
            return false; // Non-stackable items (equipment, emblems, stickers) always get new slot
        }
        
        // Check if item already exists in inventory
        $sql = "SELECT pii.id, pii.idSlot, pii.quantidade, pi.slot
                FROM personagens_inventario_itens as pii
                INNER JOIN personagens_inventario as pi ON pi.id = pii.idSlot
                WHERE pii.idPersonagem = ? AND pii.idItem = ?
                ORDER BY pi.slot ASC
                LIMIT 1";
        $stmt = DB::prepare($sql);
        $stmt->execute([$idPersonagem, $item->id]);
        $existing = $stmt->fetch();
        
        if($existing && $existing->quantidade < 99) {
            // Return existing slot for stacking (max 99)
            return $existing->idSlot;
        }
        
        return false; // Item doesn't exist or stack is full
    }
