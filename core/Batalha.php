...
class Batalha {
    ...
    public function atack($idAtack, $meuID, $idAdversario, $desafiante, $finalizado = 0){
        ...
        //ADVERSARIO ATACA NOVAMENTE (removed recursive call - will happen on page refresh)
        if($dados_pvp->atacou == 1 && $dados_pvp->atacado == 0){
            if($life_oponente <= 0){
                $finalizado = 1;
            }

            // Counter-attack removed - will be handled by checking the atacou/atacado flags on page load
        }
    }
