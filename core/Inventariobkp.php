    ...
    public function getSlots($idPersonagem){
        ...
        foreach ($item as $key => $value) {
            ...
            if($guardados->total > 0){
                ...
                // ✅ REMOVED inline style - let CSS handle backgrounds
                ...
                if($produto->hp > 0) $row .= '<p><strong>HP:</strong>+ '.$produto->hp.$percent.'</p>';
                if($produto->mana > 0) $row .= '<p><strong>KI:</strong>+ '.$produto->mana.$percent.'</p>';
                if($produto->energia > 0) $row .= '<p><strong>Energia:</strong>+ '.$produto->energia.'</p>';
                if($produto->forca > 0) $row .= '<p><strong>Força:</strong>+ '.$produto->forca.'</p>';
                if($produto->agilidade > 0) $row .= '<p><strong>Agilidade:</strong>+ '.$produto->agilidade.'</p>';
                if($produto->habilidade > 0) $row .= '<p><strong>Habilidade:</strong>+ '.$produto->habilidade.'</p>';
                if($produto->resistencia > 0) $row .= '<p><strong>Resistência:</strong>+ '.$produto->resistencia.'</p>';
                if($produto->sorte > 0) $row .= '<p><strong>Sorte:</strong>+ '.$produto->sorte.'</p>';
                
                $row .= '</div>';
                $row .= '</li>';
            } else {
                // ✅ Empty slot - CSS will handle background
    ...
    public function getSlotsEquipados($idPersonagem){
        ...
        foreach ($item as $key => $value) {
            if($value->emblema == 1 && $value->adesivo == 0){ 
                ...
                $row .= '<li class="slots slot-equipado slot-4"></li>';
            }
        }
