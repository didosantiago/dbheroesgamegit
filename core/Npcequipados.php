...
class Npc {
    ...
    public function atack($idAtack, $meuID, $idAdversario, $desafiante, $finalizado = 0){
        ...
        $log_html = $campos['log'];
        $sql_insert = "INSERT INTO npc_historico (idNPC, dano_personagem, dano_adversario, round, log" . 
                    ($desafiante == 0 ? ", ki_usado" : "") . ") VALUES (?, ?, ?, ?, ?" . 
                    ($desafiante == 0 ? ", ?" : "") . ")";
        ...
        if($desafiante == 0){
            $stmt_insert->execute([
                $dados_npc->id, 
                $campos['dano_personagem'], 
                $campos['dano_adversario'], 
                $round, 
                $log_html,
                $campos['ki_usado']
            ]);
        ...
        // Get the correct NPC HP considering VIP/Free reduction
        if($desafiante == 0){
            $life_oponente = $dados_atacante->hp - $lifes->dano_atacante;
            $ki_oponente = $dados_atacante->ki - $kis->ki_npc;
        } else {
            // Apply the same HP reduction as shown to the player
            $vip = 1; // Get from session or user data
            if($vip == 1){
            ...
            $life_oponente = $hp_npc - $lifes->dano_atacado;
        }



        // ✅ CHECK DEFEAT CONDITION AND END BATTLE IMMEDIATELY
        if($life_oponente <= 0){
            // Someone died - end the battle NOW before counter-attacking
            $vencedor = ($desafiante == 1) ? $meuID : $idAdversario;
            
            $campos_up = array(
                'concluido' => 1,
                'vencedor' => $vencedor
            );
            $where_up = 'id = '.$dados_npc->id;
            $core->update('npc', $campos_up, $where_up);
            
            // Set session variables for victory/defeat screen
            if($desafiante == 1){
                // Player won
            ...
            return; // ✅ STOP HERE - Battle is over, do NOT counter-attack
        }

        // ✅ NPC COUNTER-ATTACK (Only if battle is still active)
        if($desafiante == 1){
            // Player just attacked, trigger NPC counter-attack immediately
            $this->atack(
                4,              // Soco (basic attack)
                $idAdversario,  // NPC attacks
                $meuID,         // Player defends
                0,              // desafiante = 0 (NPC turn)
                0               // Not finished yet
            );
        }
