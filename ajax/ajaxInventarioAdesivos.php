...
try {
    ...
    if(!$item || $item->adesivo != 1){
        echo json_encode(['success' => false, 'message' => 'Item não é um adesivo']);
        exit;
    }
    
    // Equip/unequip adesivo
    $result = $inventario->equiparAdesivo($idItem, $idPersonagem);
    if($result){
        $inventario->getSlotsAdesivos($idPersonagem);
    } else {
        echo json_encode(['success' => false, 'message' => 'Erro ao equipar adesivo']);
    }
